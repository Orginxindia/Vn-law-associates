    ...
    <section id="services" className="py-24 md:py-32 relative bg-neutral-950 text-white overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 lg:px-12 relative z-10">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 text-neutral-350 text-xs font-bold tracking-widest uppercase mb-6 mx-auto shadow-md">
            <span className="w-2 h-2 rounded-full bg-white animate-pulse"></span>
            Our Practice Areas
          </div>
          <h2 className="font-bricolage font-bold text-3xl md:text-4xl lg:text-5xl tracking-tight leading-[1.1] mb-6">
            Comprehensive Legal <span className="bg-gradient-to-r from-white via-neutral-200 to-neutral-400 bg-clip-text text-transparent">Services</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {SERVICES.map((svc) => (
            <div key={svc.title} className="group relative bg-white/[0.02] hover:bg-white/[0.04] border border-white/5 hover:border-white/20 rounded-3xl overflow-hidden transition-all duration-500 hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-white/[0.01] shine-effect">
              <div className="p-6 sm:p-8">
                <div className="flex items-start justify-between mb-6">
                  <div className="w-[52px] h-[52px] rounded-xl flex items-center justify-center bg-white/5 border border-white/10 text-neutral-300 group-hover:text-white text-xl transition-all duration-300 group-hover:scale-105 group-hover:bg-white/10 group-hover:border-white/20">
                    <i className={`fas ${svc.icon}`}></i>
                  </div>
                  <span className="opacity-0 group-hover:opacity-100 translate-x-[-6px] group-hover:translate-x-0 transition-all duration-300 text-white text-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  {svc.tags.map((tag) => (
                    <span key={tag} className="text-[11px] text-neutral-400 bg-white/5 border border-white/5 rounded-full px-3 py-1 transition-colors duration-300 group-hover:text-white group-hover:border-white/10">{tag}</span>
                  ))}
                </div>
              </div>
            </div>
